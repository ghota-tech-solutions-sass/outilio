...
// Taux fallback (taux BCE du 23/09/2026 - mis a jour dynamiquement via frankfurter / BCE)
const FALLBACK_DATE = "2026-09-23";
const FALLBACK_CURRENCIES: Currency[] = [
  { code: "EUR", name: "Euro", symbol: "€", flag: "\u{1F1EA}\u{1F1FA}", rateToEUR: 1 },
  { code: "USD", name: "Dollar américain", symbol: "$", flag: "\u{1F1FA}\u{1F1F8}", rateToEUR: 1.1411 },
  { code: "GBP", name: "Livre sterling", symbol: "\u00A3", flag: "\u{1F1EC}\u{1F1E7}", rateToEUR: 0.8595 },
  { code: "CHF", name: "Franc suisse", symbol: "CHF", flag: "\u{1F1E8}\u{1F1ED}", rateToEUR: 0.939 },
  { code: "CAD", name: "Dollar canadien", symbol: "C$", flag: "\u{1F1E8}\u{1F1E6}", rateToEUR: 1.6077 },
  { code: "JPY", name: "Yen japonais", symbol: "\u00A5", flag: "\u{1F1EF}\u{1F1F5}", rateToEUR: 180.2 },
  { code: "AUD", name: "Dollar australien", symbol: "A$", flag: "\u{1F1E6}\u{1F1FA}", rateToEUR: 1.6146 },
  { code: "CNY", name: "Yuan chinois", symbol: "¥", flag: "\u{1F1E8}\u{1F1F3}", rateToEUR: 7.6538 },
  { code: "MAD", name: "Dirham marocain", symbol: "MAD", flag: "\u{1F1F2}\u{1F1E6}", rateToEUR: 10.85, staticNote: "taux indicatif (hors API BCE)" },
  { code: "XOF", name: "Franc CFA (BCEAO)", symbol: "CFA", flag: "\u{1F1F8}\u{1F1F3}", rateToEUR: 655.957, fixed: true, staticNote: "taux fixe par accord monétaire avec le Trésor français" },
];
...
export default function CalculateurTauxChange() {
  ...
  useEffect(() => {
    ...
    // api.frankfurter.app redirige (301 sans en-tetes CORS) : appeler directement le nouveau domaine
    fetch("https://api.frankfurter.dev/v1/latest?from=EUR", { signal: ctrl.signal })
      .then((r) => {
  ...
  const swap = () => {
    ...
    if (result) {
      // Valeur brute (point decimal) : formatNumber produit "1 234,56", illisible par parseFloat / input number
      setAmount(String(Math.round(result.converted * 100) / 100));
    }
  };
      ...
      <section className="relative py-14" style={{ borderBottom: "1px solid var(--border)" }}>
        <div className="mx-auto max-w-7xl px-6 2xl:max-w-[1400px]">
          ...
          <p className="animate-fade-up stagger-2 mt-3 max-w-xl text-sm leading-relaxed" style={{ color: "var(--muted)" }}>
            ...
            {ratesDate ? (
              <span>Taux BCE du <strong className="text-[var(--foreground)]">{new Date(ratesDate + "T00:00:00").toLocaleDateString("fr-FR")}</strong> (via frankfurter).</span>
            ) : apiError ? (
              <span>Taux indicatifs du {new Date(FALLBACK_DATE + "T00:00:00").toLocaleDateString("fr-FR")} (API indisponible).</span>
            ) : (
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="rounded-2xl border p-8" style={{ background: "var(--surface)", borderColor: "var(--border)" }}>
              <h2 className="text-2xl tracking-tight" style={{ fontFamily: "var(--font-display)" }}>À propos des taux de change</h2>
              <div className="mt-4 space-y-3 text-sm leading-relaxed" style={{ color: "var(--muted)" }}>
                <p><strong className="text-[var(--foreground)]">Source des taux</strong> : Les taux affichés sont les taux de référence de la <strong className="text-[var(--foreground)]">Banque Centrale Européenne (BCE)</strong>, récupérés en temps réel via l&apos;API publique Frankfurter (api.frankfurter.dev). Ils sont mis à jour chaque jour ouvré vers 16h CET. Les taux réels varient selon les banques et les plateformes de change.</p>
                <p><strong className="text-[var(--foreground)]">Franc CFA</strong> : Le XOF (Franc CFA de la BCEAO) a un taux fixe de 655,957 pour 1 EUR, garanti par le Trésor français via l&apos;accord de coopération monétaire. Le dirham marocain (MAD) n&apos;est pas couvert par l&apos;API BCE : son taux est indicatif.</p>
                <p><strong className="text-[var(--foreground)]">Frais de change</strong> : Les banques appliquent généralement une marge de 1-3% sur le taux interbancaire. Pour les meilleurs taux, comparez les services de transfert en ligne.</p>
              </div>
            </div>
            ...
            <div className="rounded-2xl border p-8" style={{ background: "var(--surface)", borderColor: "var(--border)" }}>
              ...
              <div className="mt-4 space-y-3 text-sm leading-relaxed" style={{ color: "var(--muted)" }}>
                <p>Notre convertisseur de devises vous permet de calculer instantanément la valeur d&apos;un montant dans une autre monnaie. Idéal pour préparer un voyage, vérifier un prix en ligne ou comparer des offres internationales.</p>
                <ul className="ml-4 list-disc space-y-1">
                  <li><strong className="text-[var(--foreground)]">Sélectionnez les devises</strong> : choisissez la devise de départ et la devise d&apos;arrivée parmi EUR, USD, GBP, CHF, CAD, JPY, AUD, CNY, MAD et XOF.</li>
                  <li><strong className="text-[var(--foreground)]">Entrez le montant</strong> : saisissez la somme à convertir. Le résultat s&apos;affiche en temps réel.</li>
                  <li><strong className="text-[var(--foreground)]">Inversez en un clic</strong> : utilisez le bouton d&apos;inversion pour échanger les devises instantanément.</li>
            ...
            <div className="rounded-2xl border p-8" style={{ background: "var(--surface)", borderColor: "var(--border)" }}>
              ...
              <div className="mt-6 space-y-5">
                <div className="rounded-xl p-5" style={{ background: "var(--surface-alt)" }}>
                  ...
                  <p className="mt-2 text-sm leading-relaxed" style={{ color: "var(--muted)" }}>Les banques et bureaux de change appliquent une marge (spread) sur le taux interbancaire, qui est le taux de référence entre banques. Cette marge, généralement de 1 à 3%, constitue leur rémunération. Les services de transfert en ligne comme Wise ou Revolut proposent souvent des marges plus faibles que les banques traditionnelles.</p>
                </div>
                <div className="rounded-xl p-5" style={{ background: "var(--surface-alt)" }}>
                  <h3 className="text-sm font-semibold" style={{ color: "var(--foreground)" }}>Qu&apos;est-ce que le taux fixe EUR/XOF ?</h3>
                  <p className="mt-2 text-sm leading-relaxed" style={{ color: "var(--muted)" }}>Le Franc CFA (XOF) de la zone BCEAO (Afrique de l&apos;Ouest) a un taux de change fixe avec l&apos;euro : 1 &euro; = 655,957 XOF. Ce taux est garanti par le Trésor français depuis la création de la zone franc. Cela signifie que ce taux ne fluctue pas sur les marchés, contrairement aux autres devises.</p>
                </div>
                <div className="rounded-xl p-5" style={{ background: "var(--surface-alt)" }}>
                  ...
                  <p className="mt-2 text-sm leading-relaxed" style={{ color: "var(--muted)" }}>Évitez les bureaux de change des aéroports qui pratiquent des marges élevées. Privilégiez le paiement par carte bancaire sans frais de change (type Revolut, N26 ou Boursorama Ultim), ou retirez dans des distributeurs locaux. Comparez toujours plusieurs options et refusez la &laquo; conversion dynamique &raquo; (DCC) proposée par certains terminaux de paiement à l&apos;étranger.</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <aside className="space-y-6">
            ...
            <div className="rounded-2xl border p-6" style={{ background: "var(--surface)", borderColor: "var(--border)" }}>
              ...
              <ul className="mt-3 space-y-2 text-xs leading-relaxed" style={{ color: "var(--muted)" }}>
                <li><strong>EUR/USD</strong> : Paire la plus échangée au monde</li>
                <li><strong>EUR/GBP</strong> : Échanges européens</li>
