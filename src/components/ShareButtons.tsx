...
export default function ShareButtons() {
  ...
  const current = useMemo(
    () => tools.find((t) => t.href === pathname),
    [pathname]
  );

  if (!current) return null;

  const url = `${BASE_URL}${current.href}`;
  const text = `${current.title} - Outil gratuit en ligne`;
  ...
  return (
    <div className="no-print mx-auto max-w-5xl px-5 pb-2">
      ...
      >
        <span className="mr-1 text-xs font-semibold uppercase tracking-wider" style={{ color: "var(--muted)" }}>
          Partager
        </span>
        ...
        <button
          ...
          style={{
            ...
            background: copied ? "var(--surface-alt)" : "transparent",
          }}
